...
Create a voltage divider circuit combining a flex sensor 
with a 47k resistor.
- The resistor should connect from A0 to GND.
- The flex sensor should connect from A0 to 3.3V
As the resistance of the flex sensor increases (meaning it's being bent), 
the voltage at A0 should decrease. Transmit to BLE centrals
********************************************************************/
// Include BLE files.
#include <CurieBLE.h>
...
BLEService sensingService("181A"); 

// BLE Battery Level Characteristic
BLEUnsignedIntCharacteristic windDirChar("2A73", // 16-bit characteristic UUID
    BLERead | BLENotify);     
// remote clients will be able to get notifications 
// if this characteristic changes

//###Flex sensor properties###
const int FLEX_PIN = A0; // Pin connected to voltage divider output
const int LED_PIN = 13;

// Measure the voltage at 5V and the actual resistance of your 47k resistor, 
...
void setup() 
{
  ...
  // initialize the LED to indicate when a central is connected
  pinMode(LED_PIN, OUTPUT);
  Serial.println("Attached flex pin");
  /* Set a local name for the BLE device
     This name will appear in advertising packets
     and can be used by remote devices to identify this BLE device
     The name can be changed but maybe be truncated based on space left 
   in advertisement packet */
  blePeripheral.setLocalName("FlexSensor1");
  blePeripheral.setAdvertisedServiceUuid(
  sensingService.uuid()); // add the service UUID
  blePeripheral.addAttribute(
  sensingService);   // Add the BLE Battery service
  blePeripheral.addAttribute(
  windDirChar); // add the battery level characteristic
  windDirChar.setValue(
  oldFlexAngle);   // initial value for this characteristic
  while(!Serial);
  Serial.println("Configured BLE");
  /* Now activate the BLE device. It will start continuously transmit BLE
...
void loop() 
{
  ...
  if (central) {
    ...
    // check the flex angle every 200ms 
...
void updateFlexAngle() {
  ...
  int angle = (int) map(flexR, STRAIGHT_RESISTANCE, 
  BEND_RESISTANCE, 0, 90.0);
